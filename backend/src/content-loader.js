...
const countryContentProfilesPath = path.join(backendRoot, "data", "country-content-profiles.json");
let countryContentProfilesCache = null;

function loadCountryContentProfiles() {
  if (countryContentProfilesCache) return countryContentProfilesCache;
  if (!fs.existsSync(countryContentProfilesPath)) {
    countryContentProfilesCache = {};
    return countryContentProfilesCache;
  }
  const profiles = readJson(countryContentProfilesPath);
  countryContentProfilesCache = profiles && typeof profiles === "object" && !Array.isArray(profiles) ? profiles : {};
  return countryContentProfilesCache;
}
...
function languageFallbackOrder(language, fallbackLanguage) {
  ...
  if (requested === "hy") return unique([requested, fallbackLanguage, "ru", "en"]);
...
function countryContentProfile(countryId) {
  const profiles = loadCountryContentProfiles();
  return profiles[String(countryId || "").trim().toLowerCase()] || null;
}
